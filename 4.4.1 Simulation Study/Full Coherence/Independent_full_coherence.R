...
write.table(MC_coherence, file="Full_Corr_1000_4.txt")
...
plot(omega, c, type='l', ylim=c(0,1), xlab=expression(omega), ylab=expression(hat(R["12"])(omega)))
lines(omega, c2, col='red')
lines(omega, c3, col='red')
legend("topright", c("Estimated Coherence", "95% Confidence Interval"), col=c("black", "red"), lty=c(1,1), cex=0.7)
...
k.sections=4
MC_coherence4 = matrix(NA, nrow = 256, ncol=1000)
for(i in 1:1000){
  T_s<-simulateHawkes(lambda0,alpha,beta,T_end)
  groups_new = groups(k.sections)
  group1=groups_new[[1]]
  group2=groups_new[[2]]
  MC_coherence4[,i] = periodogram_function(group1,group2, k.sections,T_s)[[3]]
  print(i)
}

write.table(MC_coherence4, file="Full_Corr_1000_4.txt")
r=apply(MC_coherence4, 1, mean) #point wise average
r2 = rowQuantiles(Re(as.matrix(MC_coherence4)), probs=0.975)
r3 = rowQuantiles(Re(as.matrix(MC_coherence4)), probs=0.025)
plot(omega, r, type='l', ylim=c(0,1), xlab=expression(omega), ylab=expression(hat(R["12"])(omega)))
lines(omega, r2, col='red')
lines(omega, r3, col='red')
legend("topright", c("Estimated Coherence", "95% Confidence Interval"), col=c("black", "red"), lty=c(1,1), cex=0.7)

k.sections = 4
MC_coherence_2 = matrix(NA, nrow = 256, ncol=1000)
for(i in 1:1000){
  T_s<-simulateHawkes(lambda0,alpha,beta,T_end)
  groups_new = groups(k.sections)
  group1=groups_new[[1]]
  group2=groups_new[[2]]
  MC_coherence_2[,i] = periodogram_function(group1,group2, k.sections,T_s)[[3]]
  print(i)
}
write.table(MC_coherence_2, file="Full_Corr_10000_4.txt")
